...
public class AccesLocal {
    ...
    public Recette recupDernierEnreg(){
        ...
        if (!curseur.isAfterLast()){
            ...
            donnee = new Recette(volumeBiere,  alcoolDegre,  moyenneEBC);
        }
        curseur.close();
    ...
    public ArrayList<Recette> recupEtoile(){
        ArrayList<Recette> listDonnee = new ArrayList<>();
        database = accesBD.getReadableDatabase();
        String request = "select * from BddRecette;";
        Cursor curseur = database.rawQuery(request, null);
        Recette donnee = null;
        ...
        while (curseur.isAfterLast() == false)
        {
            ...
            donnee = new RecetteBDD(id ,volumeBiere,  alcoolDegre,  moyenneEBC);
